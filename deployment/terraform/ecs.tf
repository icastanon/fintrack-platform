...
resource "aws_ecs_task_definition" "worker" {
  family                   = "${local.resource_prefix}-worker-service"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = "512"
  memory                   = "1024"
  ...
  container_definitions = jsonencode([
    {
      name      = "worker-service"
      image     = "${aws_ecr_repository.service["worker-service"].repository_url}:${var.worker_image_tag}"
      essential = true
      ...
      logConfiguration = {
        ...
        options = {
          awslogs-group         = aws_cloudwatch_log_group.ecs_service["worker-service"].name
          awslogs-region        = var.aws_region
  ...
  tags = {
    Name = "${local.resource_prefix}-worker-service"
  }
}